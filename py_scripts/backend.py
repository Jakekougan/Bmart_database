...
def get_connection() -> mysql.connector.connection:
    ...
    try:
        cnx =  mysql.connector.connect(user='afhj', password='Titans25', host='cs314.iwu.edu', database='afhj')
    ...
    try:
        ...
        with c.cursor() as cursor:
            for reorder in reorders:
                query = f"SELECT * from reorder_requests JOIN bmart_products ON reorder_requests.product = bmart_products.upc WHERE reorder_id = {reorder}"
            ...
            '''ship_q = "INSERT INTO shipment (`estimated_delivery`, `delivered`, `store`, `vendor`, `request`, `product_num`)" \
                f"VALUES ({deliverydate}, 0, {store}, {reorder} )"'''
            print(reorder_data)
